...
std::shared_ptr<ov::Model> build_conv_add_softmax_conv_equivalent_model(size_t sq, size_t sk, size_t d, size_t ev) {
    ...
    auto k_w = std::make_shared<op::v1::Reshape>(k_2d, i64_const({static_cast<int64_t>(sk), static_cast<int64_t>(d), 1, 1}), false);
    ...
    auto v_w = std::make_shared<op::v1::Reshape>(v_w2d, i64_const({static_cast<int64_t>(ev), static_cast<int64_t>(sk), 1, 1}), false);
