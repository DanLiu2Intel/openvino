...
function(_ov_add_api_validator_post_build_step_recursive)
    ...
    message("3-" ${API_VALIDATOR_TARGETS})
endfunction()

message("0.1 - " ${VALIDATED_TARGETS})
set(VALIDATED_TARGETS "" CACHE INTERNAL "")
message("0.2 - " ${VALIDATED_TARGETS})
...
function(_ov_add_api_validator_post_build_step)
    ...
    message("0.3 - " ${API_VALIDATOR_TARGET})
    message("0.4 - " ${VALIDATED_TARGETS})
    # collect targets
    _ov_add_api_validator_post_build_step_recursive(TARGET ${API_VALIDATOR_TARGET})
    if (API_VALIDATOR_EXTRA) #THIS PART will not be called, no used as thos part.
    ...
    message("0.5 - " ${API_VALIDATOR_TARGET})
    message("0.6 - " ${VALIDATED_TARGETS})
    ...
    # apply check
    macro(api_validator_get_target_name) # get target location
        get_target_property(is_imported ${target} IMPORTED)
        message("import: (1) " ${is_imported} " (2) " ${target})
        get_target_property(orig_target ${target} ALIASED_TARGET)
        message("ALIASED_TARGET: (2) " ${is_imported} " (2) " ${target})
        if(is_imported)
            message("1.1-is_imported")
            get_target_property(imported_configs ${target} IMPORTED_CONFIGURATIONS)
            message(STATUS "2 Imported Configurations for ${target}: ${imported_configs}")  ##
    ...
    message("0.7 - " ${API_VALIDATOR_TARGETS})
