...
std::shared_ptr<ov::ICompiledModel> Plugin::compile_model(const std::shared_ptr<const ov::Model>& model,
    ...
    try {
        ...
        if (!localConfig.get<WEIGHTLESS_BLOB>()) {
            std::cout << "------(0.1) Plugin:: compile_model graph -------, localConfig is " << localConfig.toString()
                      << std::endl;

            graph = compiler->compile(successfullyDebatched ? batchedModel : model->clone(), localConfig);
        } else {
            std::cout << "------(0.2) Plugin:: compile_model graph -------, localConfig is " << localConfig.toString()
                      << std::endl;
