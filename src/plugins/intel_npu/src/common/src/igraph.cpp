...
IGraph::IGraph(ze_graph_handle_t handle, NetworkMetadata metadata, const Config& config, std::optional<ov::Tensor> blob)
    : _handle(handle),
      _metadata(std::move(metadata)), /// for vcl part, metadata is empty
      _blob(std::move(blob)),
      _logger("IGraph", config.get<LOG_LEVEL>()) {}

const NetworkMetadata& IGraph::get_metadata() const { /// for vcl part, metadata is empty
    //any graph.get_metadata will get issue, if use vcl compile
    return _metadata;
}
