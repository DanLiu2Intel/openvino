...
public:
    ...
    bool isGraphFinishedInitialize() {
        std::cout << " ====IGraph isGraphFinishedInitialize called, _finishedInitialize is " << _finishedInitialize << std::endl;
...
protected:
    ...
    std::mutex _mutex;
    bool _finishedInitialize = false; // to show the status for graph initialization result
};
