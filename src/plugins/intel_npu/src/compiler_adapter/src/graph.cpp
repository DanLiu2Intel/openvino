...
const char* structure_type_to_string(ze_structure_type_graph_ext_t stype) {
    ...
    case ZE_STRUCTURE_TYPE_GRAPH_INPUT_HASH:
        return "ZE_STRUCTURE_TYPE_GRAPH_INPUT_HASH";
    case ZE_STRUCTURE_TYPE_GRAPH_FORCE_UINT32:
...
const char* init_stage_to_string(ze_graph_init_stage_t stage) {
    ...
    case ZE_GRAPH_STAGE_COMMAND_LIST_INITIALIZE:
        return "ZE_GRAPH_STAGE_COMMAND_LIST_INITIALIZE";
    case ZE_GRAPH_STAGE_INITIALIZE:
        return "ZE_GRAPH_STAGE_INITIALIZE";
    default:
        return "UNKNOWN";
    }
}
...
void print_ze_graph_properties_2(const ze_graph_properties_2_t& props) {
    ...
    std::cout << std::left << std::setw(30) << "Member Name" << std::setw(40) << "Type"
              << "Value" << std::endl;
    std::cout << std::string(100, '-') << std::endl;

    // stype
    std::cout << std::setw(30) << "stype" << std::setw(40) << "ze_structure_type_graph_ext_t" << props.stype << " ("
              << structure_type_to_string(props.stype) << ")" << std::endl;

    // pNext
    std::cout << std::setw(30) << "pNext" << std::setw(40) << "void*" << props.pNext;
    ...
    // numGraphArgs
    std::cout << std::setw(30) << "numGraphArgs" << std::setw(40) << "uint32_t" << props.numGraphArgs << std::endl;

    // initStageRequired
    std::cout << std::setw(30) << "initStageRequired" << std::setw(40) << "ze_graph_init_stage_t"
...
bool Graph::release_blob(const Config& config) {
    ...
    std::cout << "------(4.1)- _zeroInitStruct is ->getGraphDdiTable().version()  is "
              << _zeroInitStruct->getGraphDdiTable().version() << std::endl;
    std::cout << "------(4.2)- ZE_MAKE_VERSION(1, 8) " << ZE_MAKE_VERSION(1, 8) << std::endl;
    std::cout << "------(4.3)- _zeroInitStruct->getGraphDdiTable().version() < ZE_MAKE_VERSION(1, 8) is  "
              << (_zeroInitStruct->getGraphDdiTable().version() < ZE_MAKE_VERSION(1, 8)) << std::endl;
    ...
    std::cout << "----------(10) ~properties.initStageRequired & ZE_GRAPH_STAGE_INITIALIZE is "
              << (~properties.initStageRequired & ZE_GRAPH_STAGE_INITIALIZE) << std::endl;
