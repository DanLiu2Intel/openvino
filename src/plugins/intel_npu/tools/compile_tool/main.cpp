...
int main(int argc, char* argv[]) {
    try {
        ...
        const char* runSingle = std::getenv("RUN_MULTI_THREAD_TEST_FOR_SAME_MODEL");

        if (runSingle) {
            //use the model parse form compile_tool.exe cmd
            std::string device = FLAGS_d;
            std::vector<std::thread> threads;
            for (int i = 0; i < 3; i++){
                std::cout << "    [ INFO ] before go into thread : model name is" << model->get_name() << std::endl;
                threads.emplace_back([&core, model, device, &configs, i] {
                    std::cout << "    [ INFO ] in thread[" << i << "], model name is" << model->get_name() << std::endl;
                    printCurrentCpu();
